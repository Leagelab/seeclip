...
def train_epoch(model, unknown_image_generator, domainnames, train_loader, optimizer, lr_scheduler, step,
    ...
    """
    SeeCLIP multi-domain training function, integrating semantic-aware prompt enhancement and complete loss optimization system
    
    Args:
        model: SeeCLIP model instance
        unknown_image_generator: Pseudo-unknown sample generator
        domainnames: List of domain names, expected to contain 3 domains
    ...
    Returns:
        tuple: (Loss recorder, average accuracy)
    """
    model.train()
    ...
    for img_prev, domain_prev, label_prev, label_one_hot_prev in tqdm_object:
        # Data preprocessing and device transfer
        ...
        with torch.no_grad():
            temp_results = model(img_prev[:min(4, batch)], label_prev[:min(4, batch)])
            if 'semantic_tokens' in temp_results:
                semantic_tokens = temp_results['semantic_tokens']
            else:
                # If the model does not return semantic tokens, use a placeholder
                semantic_tokens = torch.randn(min(4, batch), 8, 512).to(device)
        
        # Generate semantic-guided pseudo-unknown samples for each domain
        ...
        # Build unknown class labels and domain labels
        ...
        # Combine pseudo-unknown samples generated from multiple domains
        ...
        # Combine known samples with filtered unknown samples
        ...
        if hasattr(model, 'forward') and len(inspect.signature(model.forward).parameters) > 2:
            # If the model supports the SeeCLIP interface
            results = model(img, label)
            if isinstance(results, dict):
                output = results.get('logits', results.get('output'))
                losses = results.get('losses', {})
                diff_projfeatures = results.get('diff_projfeatures', results.get('semantic_tokens'))
            else:
                output, diff_projfeatures = results if isinstance(results, tuple) else (results, None)
                losses = {}
        else:
            # Traditional model interface
            result = model(img, label)
            if isinstance(result, tuple):
                output, diff_projfeatures = result
            else:
                output = result
                diff_projfeatures = None
            losses = {}
        
        output = output.to(device)
        
        # Compute base cross-entropy loss (alignment loss)
        crossentropy_loss = F.cross_entropy(output, label)
        alignment_loss = losses.get('alignment', crossentropy_loss)
        
        # Compute SeeCLIP-specific loss components
        ...
        # Compute domain text loss
        if diff_projfeatures is not None:
            ...
            text_loss = torch.tensor(0.0).to(device)
        ...
        # Learning rate scheduling update
        if step == "batch":
...
def train_epoch_with_enhanced_semantic_guidance(model, unknown_image_generator, domainnames, train_loader, optimizer, lr_scheduler, step,
                                               alpha=1.0, beta=0.5, gamma=0.01, sigma=0.1):
    """
    Enhanced SeeCLIP training function, including explicit semantic token perturbation and guidance
    
    This version implements the complete semantic-guided diffusion mechanism as described in the paper,
    including semantic token perturbation and conditional generation
    """
    model.train()
    ...
    for img_prev, domain_prev, label_prev, label_one_hot_prev in tqdm_object:
        ...
        # Step 1: Extract and perturb semantic tokens
        with torch.no_grad():
            temp_results = model(img_prev[:min(4, batch)], label_prev[:min(4, batch)])
            if 'semantic_tokens' in temp_results:
                semantic_tokens = temp_results['semantic_tokens']
                # Apply Gaussian noise perturbation (Equation 8)
                noise = torch.normal(0, sigma, size=semantic_tokens.shape).to(semantic_tokens.device)
                perturbed_semantic_tokens = semantic_tokens + noise
            else:
                perturbed_semantic_tokens = torch.randn(min(4, batch), 8, 512).to(device)
        
        # Step 2: Generate semantic-guided pseudo-unknown samples using perturbed semantic tokens
        generated_images_list = []
        unknown_domains_list = []
        
        for domain_idx, domain_name in enumerate(domainnames):
            # Build positive condition prompt
            positive_prompt = f"A {domain_name.replace('_', ' ')} image of an unknown class"
            
            # Build negative condition prompt (including all known classes)
            negative_prompt = ", ".join(known_classes) if 'known_classes' in globals() else ""
            ...
            if hasattr(model, 'diffusion_generator'):
                generated_images = model.diffusion_generator.generate_pseudo_unknowns(
                    ...
                    known_classes if 'known_classes' in globals() else [],
                    batch_size=max(1, int(batch * 0.1))
                )
            else:
                # Fallback: Use traditional generator method
                generated_images = unknown_image_generator(batch, positive_prompt, known_classes if 'known_classes' in globals() else [])
        ...
        # Step 3: Process and merge generated samples
        generated_unknown_images = torch.cat(generated_images_list, dim=0)
        ...
        # Step 4: Full SeeCLIP forward pass and loss computation
        results = model(img, label)
        ...
        alignment_loss = losses.get('alignment', F.cross_entropy(output, label))
        repulsion_loss = losses.get('repulsion', torch.tensor(0.0).to(device))
        cohesive_loss = losses.get('cohesive', torch.tensor(0.0).to(device))
        reg_loss = losses.get('regularization', torch.tensor(0.0).to(device))
