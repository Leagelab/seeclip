...
def train_epoch(model, train_loader, optimizer, lr_scheduler, step, 
                alpha=1.0, beta=0.1, gamma=0.01, generate_unknowns=True):
    """
    Modified training epoch function for SeeCLIP framework
    ...
    for batch_idx, (img_prev, domain_prev, label_prev, label_one_hot_prev) in enumerate(tqdm_object):
        ...
        # Phase 1: Process known samples with SeeCLIP
        results = model(img_prev, labels=label_prev, generate_unknowns=generate_unknowns)
        
        known_logits = results['known_logits']
        unknown_logits = results['unknown_logits'] 
        all_logits = results['logits']
        losses = results['losses']
        
        # Phase 2: Generate and process pseudo-unknown samples if enabled
        if generate_unknowns and 'pseudo_unknowns' in results:
            pseudo_unknowns = results['pseudo_unknowns']
            
            # Create labels for pseudo-unknowns (unknown class index)
            unknown_label_idx = model.num_classes  # Index for unknown class
            pseudo_labels = torch.full((pseudo_unknowns.shape[0],), unknown_label_idx, 
                                     dtype=torch.long).to(device)
            
            # Process pseudo-unknowns through the model
            pseudo_results = model(pseudo_unknowns, labels=None, generate_unknowns=False)
            pseudo_logits = pseudo_results['logits']
            
            # Combine known and pseudo-unknown samples
            combined_images = torch.cat([img_prev, pseudo_unknowns], dim=0)
            combined_labels = torch.cat([label_prev, pseudo_labels], dim=0)
            combined_logits = torch.cat([all_logits, pseudo_logits], dim=0)
            
            # Compute alignment loss on combined data
            alignment_loss = F.cross_entropy(combined_logits, combined_labels)
        else:
            # Only use known samples
            alignment_loss = losses.get('alignment', F.cross_entropy(known_logits, label_prev))
        ...
        total_loss = alignment_loss + alpha * repulsion_loss + beta * cohesive_loss + gamma * reg_loss
        ...
        # Update learning rate scheduler if batch-wise
        if step == "batch":
            lr_scheduler.step()
        
        # Calculate accuracy on known classes only
        with torch.no_grad():
            # For accuracy calculation, only consider known class predictions
            known_predictions = known_logits.argmax(dim=-1)
            acc = (known_predictions == label_prev).float().mean().item() * 100
        
        # Update meters
        count = img_prev.size(0)
        ...
        tqdm_object.set_postfix({
            ...
            'ali_loss': f'{alignment_loss_meter.avg:.4f}',
            'rep_loss': f'{repulsion_loss_meter.avg:.4f}',
            'coh_loss': f'{cohesive_loss_meter.avg:.4f}',
            'reg_loss': f'{reg_loss_meter.avg:.4f}',
            'accuracy': f'{accuracy_meter.avg:.2f}%',
    ...
    return {
        ...
        'alignment_loss': alignment_loss_meter.avg,
        'repulsion_loss': repulsion_loss_meter.avg,
        'cohesive_loss': cohesive_loss_meter.avg,
        'regularization_loss': reg_loss_meter.avg,
