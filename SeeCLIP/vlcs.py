...
def train_epoch(model, unknown_image_generator, domainnames, train_loader, optimizer, lr_scheduler, step, 
    ...
    """
    SeeCLIP training function, integrating semantic-aware prompt enhancement and multi-loss optimization
    
    Args:
        model: SeeCLIP model instance
        unknown_image_generator: Generator for pseudo-unknown samples
    ...
    for img_prev, domain_prev, label_prev, label_one_hot_prev in tqdm_object:
        ...
        # Use semantic-guided diffusion to generate pseudo-unknown samples
        # First, extract the current batch's semantic tokens to guide the generation
        with torch.no_grad():
            temp_results = model(img_prev[:min(4, batch)], generate_unknowns=False)
            semantic_tokens = temp_results['semantic_tokens']
        
        # Construct semantic-guided positive prompts (with domain information)
        if len(domainnames) > 0:
            domain_name = domainnames[domain_prev[0].item()].replace("_", " ")
            unknown_posprompt = f"A {domain_name} image of an unknown object"
        else:
        ...
        # Create labels for unknown samples and domain labels
        ...
        # Forward pass to get SeeCLIP results
        ...
        # Calculate base cross-entropy loss (alignment loss)
        ...
        # Domain text loss (retaining original functionality)
        if 'diff_projfeatures' in results:
            text_loss = domain_text_loss(results['diff_projfeatures'], domain)
        else:
            # If 'diff_projfeatures' is not in the model output, use a fallback
            diff_projfeatures = results.get('semantic_tokens', torch.zeros(img.size(0), 512).to(device))
            text_loss = domain_text_loss(diff_projfeatures, domain)
        
        # Compute total loss (based on formula 11)
