...
def train_epoch(model, unknown_image_generator, domainnames, train_loader, optimizer, lr_scheduler, step, 
    ...
    """
    SeeCLIP multi-domain training function, integrating semantic-aware prompt enhancement and multi-loss optimization
    
    Args:
        model: SeeCLIP model instance
        unknown_image_generator: Pseudo-unknown sample generator
        domainnames: List of domain names [domain1, domain2, domain3]
        train_loader: Training data loader
        optimizer: Optimizer
        lr_scheduler: Learning rate scheduler
        step: Learning rate scheduling step type
        alpha: Repulsion loss weight
        beta: Cohesion loss weight
    ...
    for img_prev, domain_prev, label_prev, label_one_hot_prev in tqdm_object:
        ...
        # Extract semantic tokens for guiding pseudo-unknown sample generation
        with torch.no_grad():
            temp_results = model(img_prev[:min(4, batch)], generate_unknowns=False)
            semantic_tokens = temp_results['semantic_tokens']
        
        # Generate domain-specific pseudo-unknown samples guided by semantics
        generated_unknown_images_list = []
        unknown_domains_list = []
        
        for domain_idx, domain_name in enumerate(domainnames):
            # Build semantic-enhanced positive prompt
            unknown_posprompt = domain_name.replace("_", " ") + " of an unknown object"
            
            # Use semantic guidance to generate pseudo-unknown samples
            if hasattr(model, 'diffusion_generator'):
                # Use the SeeCLIP built-in semantic-guided diffusion generator
                domain_clean_name = domain_name.replace("_", " ")
                generated_unknown_images = model.diffusion_generator.generate_pseudo_unknowns(
                    semantic_tokens, 
                    domain_clean_name, 
                    known_classes,
                    batch_size=max(1, int(batch * 0.1))
                )
            else:
                # Use external generator but with semantic information
                generated_unknown_images = unknown_image_generator(batch, unknown_posprompt, known_classes)
            
            generated_unknown_images_list.append(generated_unknown_images)
            
            # Create corresponding domain labels
            unknown_domain_labels = torch.full((generated_unknown_images.shape[0],), domain_idx).to(device)
            unknown_domains_list.append(unknown_domain_labels)
        
        # Combine all domains' pseudo-unknown samples
        generated_unknown_images = torch.cat(generated_unknown_images_list, dim=0)
        ...
        # Image quality filtering and selection
        ...
        # SeeCLIP forward pass
        results = model(img, label)
        output = results['logits'].to(device)
        ...
        # SeeCLIP loss components
        alignment_loss = losses.get('alignment', crossentropy_loss)
        ...
        # Domain text loss calculation
        if 'diff_projfeatures' in results:
            diff_projfeatures = results['diff_projfeatures']
        elif 'semantic_tokens' in results:
            # Use semantic tokens as fallback features
            semantic_tokens_flat = results['semantic_tokens'].view(results['semantic_tokens'].shape[0], -1)
            diff_projfeatures = semantic_tokens_flat
        else:
            # Create placeholder features
            diff_projfeatures = torch.zeros(img.size(0), 512).to(device)
        
        text_loss = domain_text_loss(diff_projfeatures, domain)
        
        # Compute total loss (based on SeeCLIP formula 11)
        ...
        text_loss_meter.update(text_loss.item(), count)
        ...
        tqdm_object.set_postfix(
            ...
            align=alignment_loss_meter.avg,
            repul=repulsion_loss_meter.avg,
            cohes=cohesive_loss_meter.avg,
            reg=reg_loss_meter.avg,
            acc=accuracy_meter.avg,
            lr=get_lr(optimizer)
        )
    
    return total_loss_meter, accuracy_meter.avg


def train_epoch_with_semantic_guidance(model, unknown_image_generator, domainnames, train_loader, optimizer, lr_scheduler, step, 
                                      alpha=1.0, beta=0.5, gamma=0.01, semantic_perturbation_sigma=0.1):
    """
    Enhanced SeeCLIP training function with explicit semantic token perturbation and guidance
    """
    model.train()
    ...
    for img_prev, domain_prev, label_prev, label_one_hot_prev in tqdm_object:
        ...
        # Stage 1: Extract semantic tokens and apply perturbation
        with torch.no_grad():
            temp_results = model(img_prev, label_prev, generate_unknowns=False)
            semantic_tokens = temp_results['semantic_tokens']
            
            # Apply Gaussian noise perturbation to semantic tokens (Equation 8)
            noise = torch.normal(0, semantic_perturbation_sigma, size=semantic_tokens.shape).to(semantic_tokens.device)
            perturbed_semantic_tokens = semantic_tokens + noise
        
        # Stage 2: Generate pseudo-unknown samples for multiple domains based on perturbed semantic tokens
        generated_unknown_images_list = []
        unknown_domains_list = []
        
        for domain_idx, domain_name in enumerate(domainnames):
            # Build positive prompt
            positive_prompt = f"A {domain_name.replace('_', ' ')} image of an unknown class"
            
            # Build negative prompt (including all known classes)
            negative_prompt = ", ".join(known_classes)
            
            # Guide generation using perturbed semantic tokens
            if hasattr(model, 'diffusion_generator'):
                generated_images = model.diffusion_generator.generate_pseudo_unknowns(
                    perturbed_semantic_tokens, 
                    domain_name.replace("_", " "), 
                    known_classes,
                    batch_size=max(1, int(batch * 0.1))
                )
            else:
                # Fallback: Use traditional generator
                generated_images = unknown_image_generator(batch, positive_prompt, known_classes)
            
            generated_unknown_images_list.append(generated_images)
            unknown_domains_list.append(torch.full((generated_images.shape[0],), domain_idx).to(device))
        
        # Combine and process generated samples
        generated_unknown_images = torch.cat(generated_unknown_images_list, dim=0)
        unknown_domains = torch.cat(unknown_domains_list, dim=0)
        
        unknown_label_rank = len(train_prev_classnames)
        unknown_labels = torch.full((generated_unknown_images.shape[0],), unknown_label_rank).to(device)
        ...
        # Stage 3: Full SeeCLIP forward pass and loss computation
        results = model(img_combined, label_combined)
        output = results['logits']
        losses = results['losses']
        
        # Compute all loss components
        alignment_loss = losses.get('alignment', F.cross_entropy(output, label_combined))
        repulsion_loss = losses.get('repulsion', torch.tensor(0.0).to(device))
        cohesive_loss = losses.get('cohesive', torch.tensor(0.0).to(device))
        reg_loss = losses.get('regularization', torch.tensor(0.0).to(device))
        
        # Domain text loss
        diff_projfeatures = results.get('semantic_tokens', torch.zeros(img_combined.size(0), 512).to(device))
        if diff_projfeatures.dim() > 2:
            diff_projfeatures = diff_projfeatures.view(diff_projfeatures.shape[0], -1)
        text_loss = domain_text_loss(diff_projfeatures, domain_combined)
        ...
        acc = compute_accuracy(output, label_combined)[0].item()
        accuracy_meter.update(acc, count)
